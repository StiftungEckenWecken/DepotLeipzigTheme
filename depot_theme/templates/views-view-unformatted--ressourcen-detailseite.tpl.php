...
foreach ($resources as $resource) :
	...
	$user_is_owner = (user_has_role(ROLE_ADMINISTRATOR) || user_has_role(ROLE_REGIONALPARTNER) || $user->uid == $resource['uid']);
    ...
    if ($user->uid == $resource['uid']) {
        $breadcrumb[] = l('Meine Ressourcen', '/user/' . $user->uid . '/ressourcen');
    } else {
    ...
    <script type="text/javascript">
        ...
        var calOptions = {
            ...
            lang: 'de',
            weekStart: 1,
            button: '<?= t('Termine und Anzahl speichern'); ?>!',
    ...
    <form id="calResForm" method="GET"
          action="<?= (!user_is_logged_in() ? '/user/login?destination=/ressourcen/'. $resource['type_id'] : '/buchungen/neu') ?>">
            ...
            <div class="medium-5 columns">
                <div class="follow-box">
                    <div class="panel" itemprop="offers" itemscope itemtype="http://schema.org/Offer">
                        ...
                        <p class="text-center">
                            <span itemprop="price">
                                <?= t('Ermäßigt'); ?>: <?= $resource['field_kosten_2']; ?>
                            </span>
							<?php if (!empty($resource['field_kosten'])) : ?>
                                | <?= t('Regulär'); ?>: <?= $resource['field_kosten'].$asterisk; ?>
                            <?php endif; ?>
                            / <?= $resource['field_abrechnungstakt'].$asterisk; ?>
                            <br />

                            <?php if (empty($resource['field_kaution']) || $resource['field_kaution'] == '0,00 €') : ?>
                                <span class="kautionsfrei"
                                 style="text-align:center;font-style:italic;color:green;"
                                 title="<?= t('Diese Ressource buchst Du kautionsfrei!') ?>">
                                  <i class="fi fi-check"></i> <?= t('Kautionsfrei'); ?>!
                                </span>
                            <?php elseif (!empty($resource['field_kaution'])) : ?>
                        ...
                        <?php if (!empty($resource['field_gemeinwohl']) && $resource['field_gemeinwohl'] == 'Ja') : ?>
                            <p>
                                <span class="label warning">
                                    <?= t('Achtung!'); ?>
                                </span> <?= t('Buchungen nur durch dem Gemeinwohl verpflichtete Organisationen!'); ?>
                            </p>
                        <?php endif; ?>
                        ...
                        <?php else: 
                                ...
                                title="<?= t('Nur durch gemeinnützige Organisationen reservierbar'); ?>">
                                  <i class="fi fi-torsos-all-female"></i><?= t('Nur durch Organisationen reservierbar'); ?>
                                </a>
                            <?php else : ?>
                                <a href="#" id="availability_calendar_btn" class="button small expand ci" title="<?= t('Verfügbarkeit im gewünschten Leih-Zeitraum prüfen'); ?>">
                                  <i class="fi fi-magnifying-glass"></i><?= t('Verfügbarkeit prüfen'); ?>
                                </a>
                            <?php endif; ?>
                        <?php endif; ?>
                    ...
                    <?php endif;
                        if ((user_has_role(ROLE_ADMINISTRATOR) || user_has_role(ROLE_REGIONALPARTNER)) && $resource['user']->data['contact']) : ?>
                        <a href="/user/<?= $resource['uid']; ?>/contact"
                           id="contact_agent_btn"
                           class="button white small expand">
                           <i class="fi fi-torso"></i><?= t('Anbieter kontaktieren'); ?>
                        </a>
					<?php endif; ?>
                    ...
                    <ul id="res-detail-share-dd" class="f-dropdown" data-dropdown-content tabindex="-1"
                        ...
                        <li>
                            <a href="mailto:?subject=<?= t('Entdecke das depot!') ?>&amp;body=<?= $base_url . '/' . current_path(); ?>">E-Mail</a>
                        </li>
                    </ul>
                </div>
            </div>
